...
import { validBlogs } from "../validation/validationBlogs";
import { body, checkExact, matchedData, oneOf, validationResult } from "express-validator";
...
import { validaPosts } from "../validation/validationsPostst";
...
export const routerBlogs = () => {
  ...
  router.post("/", authMiddleware, validBlogs, async (req: Request, res: Response) => {
  ...
  router.put("/:id", authMiddleware, validBlogs, async (req: Request, res: Response) => {
    let result = await blogsService.findBlogs(req.params.id);
    if (!result) {
      res.sendStatus(SETTINGS.HTTPCOD.HTTPCOD_404);
      return;
